                    ...
                    @if ($user->id !== auth()->user()->id)
                        @if ($user->siguiendo(auth()->user()))
                            <form action="{{route("users.unfollow", $user)}}" method="POST">
                                @csrf
                                @method("DELETE")
                                <input type="submit" class="bg-red-600 text-white uppercase rounded-lg px-2 py-0.5 text-xs font-bold cursor-pointer" value="Dejar de seguir">
                            </form>
                        @else
                            <form action="{{route("users.follow", $user)}}" method="POST">
                                @csrf
                                <input type="submit" class="bg-blue-600 text-white uppercase rounded-lg px-2 py-0.5 text-xs font-bold cursor-pointer" value="Seguir">
                            </form>
                        @endif
                    @endif
                @endauth
